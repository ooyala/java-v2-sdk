...
public class SignatureGeneratorTest {
    ...
    public void testGenerateSignature() throws Exception {
        ...
        assertEquals("ualpt4pGZh9q78krXLxWOWer3lAEeY43s%2FAjawbZJeA",
                signatureGenerator.generateSignature("get", urlPath, params, "{'json': 'body', 'is': 'ok'}", expireTime));

        assertEquals("P5Ll1cxJhXhB8lMHk%2Bdaf9LAbooU7rrGRPLbrUZntXQ",
                signatureGenerator.generateSignature("get", urlPath, new HashMap<String, String>(),
                "{'json': 'body', 'is': 'ok'}", expireTime));

        assertEquals("rA9t2taPoBlqmM3Zn2UXgkYHtm5zVf21i0an4cCsPw0",
                signatureGenerator.generateSignature("get", urlPath, new HashMap<String, String>(),
                "", expireTime));
    }
